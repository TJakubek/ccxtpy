...
const fetchTokensFromMongo = async (): Promise<string[]> => {
  const mongoUrl = process.env.MONGO_SERVER || "mongodb://tsoData:naefeed0@65.109.52.201:27017";
  ...
  try {
    ...
    const tokens = [...new Set(documents.map((doc: any) => doc.symbol).filter((symbol: any) => symbol))] as string[];
